...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const { payload, targetRole = "Professional", templateId = "", usePaidCredit = false } = body;
    const email = String(body.email || payload?.personal?.email || "").trim().toLowerCase();
    if (!payload || typeof payload !== "object") {
      return res.status(400).json({ error: "Missing resume payload." });
    }
    if (usePaidCredit) {
      if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        return res.status(400).json({ error: "A valid email is required to use paid credits." });
      }
      const availableCredits = Number(await supabaseRpc("get_user_credits", { p_email: email }) || 0);
      if (availableCredits <= 0) {
        return res.status(402).json({ error: "No paid credits found for this email. Please buy credits first." });
      }
    }
...
${JSON.stringify(payload)}`;
    ...
    const model = (process.env.ANTHROPIC_MODEL || "claude-haiku-4-5-20251001").trim().replace(/^["']|["']$/g, "");
    ...
    if (usePaidCredit) {
      parsed.remainingCredits = await supabaseRpc("use_user_credit", {
        p_email: email,
        p_template_id: String(templateId || ""),
        p_target_role: String(targetRole || "Professional")
      });
    }
